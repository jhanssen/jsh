...
function fileCompletion(data, useRelative, pathsOnly)
{
    if (!data.comp.length) {
        if (!data.entry)
            return undefined;
        try {
            jsh.pathify(Tokenizer.expandTilde(data.entry.entry[0].data));
        } catch (err) {
            return undefined;
        }
    }
    if (data.comp[0] === '/' || useRelative) {
        var comp = Tokenizer.expandTilde(data.comp);
        var prefix = comp;
        ...
        if (prefix) {
            ...
            for (idx = 1; idx < cands.length; ++idx) {
...
var keywords = [ "break", "do", "instanceof", "typeof", "case", "else", "new", "var", "catch",
                 ...
                 "true", "false", "null", "undefined", "NaN", "Object", "Array", "Number", "Date" ];
function javascriptCompletion(data, current)
{
    if (current.length)
        return undefined;
    if (data.entry) {
...
module.exports = {
    ...
    fileCompletion: function(data, cur) { return completeArray(data, fileCompletion(data, true)); },
    dirCompletion: function(data, cur) { return completeArray(data, fileCompletion(data, true, true)); }
};

...
Handle<Value> ProcessChain::New(const Arguments& args)
{
    ...
    if (args.Length() != 3) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain needs a jsh object, a foreground/background argument and a callback argument")));
    }
    ...
    if (args[2].IsEmpty() || !args[2]->IsFunction()) {
        delete obj;
        return ThrowException(Exception::TypeError(String::New("ProcessChain needs a callback argument")));
    }
    obj->mCallback = Persistent<Function>::New(Handle<Function>::Cast(args[2]));
...
Handle<Value> ProcessChain::exec(const Arguments& args)
{
    ...
    if (args.Length() != 0) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain.exec takes no arguments")));
    }
...
Handle<Value> ProcessChain::cont(const Arguments& args)
{
    ...
    if (obj->mStatus != Stopped) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain.cont can't continue non-stopped chains")));
    }

...
Handle<Value> ProcessChain::cleanup(const Arguments& args)
{
    HandleScope scope;

    if (args.Length() != 0) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain.cleanup takes no arguments")));
    }

    ProcessChain* obj = ObjectWrap::Unwrap<ProcessChain>(args.This());

    if (obj->mStatus == Terminated) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain.cleanup can't cleanup terminated chains")));
    }
...
void ProcessChain::notifyStopped()
{
    ...
    obj->Set(String::New("status"), Integer::New(mStatus));

...
class ReadThread : public UVThread
{
public:
    ReadThread();
    ~ReadThread();

    void addFd(int fd, const std::function<void(const char*)>& callback);
...
private:
    UVMutex mtx;
    std::map<int, std::function<void(const char*)> > fds;
    int wakeup[2];
};
...
void ReadThread::addFd(int fd, const std::function<void(const char*)>& callback)
{
...
void ReadThread::run()
{
    fd_set rd;
    int max;
    std::map<int, std::function<void(const char*)> > local;
    for (;;) {
        FD_ZERO(&rd);
        {
            UVMutexLocker locker(mtx);
...
class WaitThread : public UVThread
{
public:
    WaitThread();
    ~WaitThread();

    bool addPid(pid_t pid, const std::function<void(pid_t pid, int status)>& callback, int* status = 0);
...
private:
    ...
    UVCondition cond;
    bool stopped;
    std::map<pid_t, std::function<void(pid_t pid, int status)> > pids;
    std::map<pid_t, int> caught;

    std::function<void(pid_t, int status)> cb;
};
...
bool WaitThread::addPid(pid_t pid, const std::function<void(pid_t pid, int status)>& callback, int* status)
{
    UVMutexLocker locker(mtx);
...
void WaitThread::run()
{
    ...
    for (;;) {
        ...
        if (pid > 0) {
            bool found = false;
            std::function<void(pid_t pid, int status)> cb;
            {
                UVMutexLocker locker(mtx);
...
void ProcessChain::init(Handle<Object> target)
{
    ...
    NODE_SET_PROTOTYPE_METHOD(constructor, "foreground", foreground);
    NODE_SET_PROTOTYPE_METHOD(constructor, "background", background);
...
Handle<Value> ProcessChain::New(const Arguments& args)
{
    ...
    mType = static_cast<Type>(Handle<Integer>::Cast(args[1])->Value());
    ...
    obj->mPgid = prop->ToInteger()->Value();
...
void ProcessChain::foreground(pid_t pgid, State state)
{
    ...
    if (state == Continue) {
        tcsetattr(STDIN_FILENO, TCSADRAIN, &mTermios);
        if (kill(-pgid, SIGCONT) < 0) {
            fprintf(stderr, "Unable to SIGCONT\n");
        }
    }
}

void ProcessChain::background(pid_t pgid, State state)
{
}
...
Handle<Value> ProcessChain::end(const Arguments& args)
{
    ...
    if (args.Length() != 1) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain.end takes a callback")));
    }
    ...
    ProcessChain* obj = ObjectWrap::Unwrap<ProcessChain>(args.This());
    if (!obj->mLaunched && !obj->launch()) {
        return ThrowException(Exception::TypeError(String::New("ProcessChain.end launch failed.")));
    }
    if (obj->mInPipe[1] != -1) {
        ::close(obj->mInPipe[1]);
        obj->mInPipe[1] = -1;
    ...
    for (;;) {
        ...
        if (pid > 0) {
            ...
            tcgetattr(STDIN_FILENO, &obj->mTermios);
            tcsetattr(STDIN_FILENO, TCSADRAIN, obj->mShellTermios);

            char buf[1024];
            snprintf(buf, 1024, "ProcessChain.exec waitpid failed (%d - %s), status %d", errno, strerror(errno), status);
...
Handle<Value> ProcessChain::foreground(const Arguments& args)
{
    mType = Foreground;
    return end(args);
}

Handle<Value> ProcessChain::background(const Arguments& args)
{
    mType = Background;
    return end(args);
}

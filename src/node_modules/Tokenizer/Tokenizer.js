...
function tokenName(tok)
{
    ...
    case QUOTEDCOMMAND: return "QUOTEDCOMMAND";
...
Object.defineProperty(Tokenizer.prototype, "line", {
    get: function line() { return this._line; }
});
...
Tokenizer.prototype._maybeAddVariable = function(entry)
{
    if (this._state.value() === DOLLAR) {
    ...
    } else if (this._var !== undefined) {
        var varname = this._line.substring(this._var + 1, this._pos);
        if (!varname.length) {
            this._var = undefined;
            return;
        }
        var prev = this._line.substring(0, this._var);
        var next = this._line.substring(this._pos);
        var varval = "";
        try {
            varval += eval(varname);
        } catch (e) {
        ...
        this._line = prev + varval + next;
        this._pos += varval.length - (varname.length + 1);
        //console.log("varname " + varname + " prev " + prev + " and next " + next);
        this._var = undefined;
    }
};
...
Tokenizer.prototype.next = function()
{
    ...
    while (!done && this._pos < this._line.length) {
        ...
        // console.log("GOT CHAR", ch, escape);
        switch (ch) {
        case '"':
            if (!escape)
        ...
        case '`':
            if (!escape)
                this._maybeAddVariable(entry);
            if (this._state.is(NORMAL)
                || (escape && this._state.is(BACKTICK))) {
                if (!escape) {
                    this._addPrev(COMMAND, entry);
                    this._addHidden(entry, "`");
                    this._state.push(BACKTICK);
                } else {
                    escape = false;
                }
            } else if (this._state.is(BACKTICK)) {
                if (!escape) {
                    this._addPrev(EXECUTE, entry);
                    this._addHidden(entry, "`");
                    this._state.pop();
                } else {
                    escape = false;

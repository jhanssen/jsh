...
JavaScript.prototype.write = function(data)
{
    ...
    if (!this._iterator) {
        ...
        ret = this._iterator.next({start: true});
        if (ret.done) {
            this._done = true;
        }
        // if (
    ...
    while (this._inPos < this._input.length) {
        var idx = this._input.indexOf(this._split, this._inPos);
        if (idx === -1)
            return;
    ...
    if (ret && ret.value) {
        if (this._next) {
            this._next.write(ret.value);
        } else if (!this._data) {
            this._data = ret.value;
        } else {
            this._data += ret.value;
        }
    }

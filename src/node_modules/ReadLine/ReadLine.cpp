...
Handle<Value> ReadLine::resume(const Arguments& args)
{
    ...
    ReadLine* obj = ObjectWrap::Unwrap<ReadLine>(args.This());
    if (args.Length() != 1) {
        return ThrowException(Exception::TypeError(String::New("ReadLine.resume takes a prompt argument")));
    }
    if (args[0].IsEmpty() || !args[0]->IsString()) {
        return ThrowException(Exception::TypeError(String::New("ReadLine.resume needs a prompt argument")));
    }
...
Handle<Value> ReadLine::New(const Arguments& args)
{
    ...
    if (args.Length() != 3) {
        return ThrowException(Exception::TypeError(String::New("ReadLine takes a prompt, one line callback and one complete callback argument")));
    }

    if (args[0].IsEmpty() || !args[0]->IsString()) {
        return ThrowException(Exception::TypeError(String::New("ReadLine needs a prompt argument")));
    }
    ...
    obj->lineCallback = Persistent<v8::Function>::New(Handle<v8::Function>::Cast(args[1]));
    obj->completeCallback = Persistent<v8::Function>::New(Handle<v8::Function>::Cast(args[2]));

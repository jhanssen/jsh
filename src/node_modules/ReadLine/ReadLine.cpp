...
static bool attemptedCompletion = false;
static int oldout = -1;
static int olderr = -1;
...
void ReadLine::RunCallback(uv_async_s* handle)
{
    SendRequest* req;
    {
...
void ReadLine::Run(uv_work_t *req)
{
    ...
    // init readline
    rl_readline_name = strdup("jsh");

    // initialize completion
    rl_attempted_completion_function = attemptShellCompletion;
    rl_completer_quote_characters = "'\"";

    rl_callback_handler_install(prompt.c_str(), handleReadLine);
...
ReadLine::ReadLine(const std::string& p)
    : ObjectWrap()
{
    setPrompt(p);
    ...
    int f;
    eintrwrap(f, fcntl(rlPipe[0], F_GETFL, 0));
    if (f != -1) {
        int f2;
        eintrwrap(f2, fcntl(rlPipe[0], F_SETFL, f | O_NONBLOCK));
    }
...
void ReadLine::setPrompt(const std::string& p)
{
    prompt = p;
    size_t idx = 0;
    for (;;) {
        idx = prompt.find("\\e", idx);
        if (idx == std::string::npos)
            break;
        prompt.replace(idx, 2, "\e");
        ++idx;
    }
}
...
char** ReadLine::handleComplete(char* text, const char* comp, int start, int end)
{
    ...
    obj->Set(NanSymbol("text"), NanSymbol(text));
    obj->Set(NanSymbol("comp"), NanSymbol(comp));
    obj->Set(NanSymbol("start"), NanNew<Integer>(start));
    obj->Set(NanSymbol("end"), NanNew<Integer>(end));

    Handle<Value> val = obj;

    auto ctx = NanGetCurrentContext();
    auto ret = NanNew<Function>(completeCallback)->Call(ctx->Global(), 1, &val);
...
void ReadLine::handleLine(char* line)
{
    ...
    Handle<Value> str;
    if (line) {
        str = NanSymbol(stripwhite(line));
    } else {
        str = NanUndefined();
    }

    auto ctx = NanGetCurrentContext();
    NanNew<Function>(lineCallback)->Call(ctx->Global(), 1, &str);
...
NAN_METHOD(ReadLine::cleanup)
{
    NanScope();

    ReadLine* obj = ObjectWrap::Unwrap<ReadLine>(args.This());
    if (args.Length() > 0) {
        return NanThrowError("ReadLine.cleanup takes no arguments");
    }
    obj->cleanup();
...
NAN_METHOD(ReadLine::resume)
{
    NanScope();

    ReadLine* obj = ObjectWrap::Unwrap<ReadLine>(args.This());
    if (args.Length() != 1) {
        return NanThrowError("ReadLine.resume takes a prompt argument");
    }
    if (args[0].IsEmpty() || !args[0]->IsString()) {
        return NanThrowError("ReadLine.resume needs a prompt argument");
    }
...
NAN_METHOD(ReadLine::New)
{
    ...
    if (args.Length() != 3) {
        return NanThrowError("ReadLine takes a prompt, one line callback and one complete callback argument");
    }

    if (args[0].IsEmpty() || !args[0]->IsString()) {
        return NanThrowError("ReadLine needs a prompt argument");
    }

    if (args[1].IsEmpty() || !args[1]->IsFunction()) {
        return NanThrowError("ReadLine needs a line callback argument");
    }

    if (args[2].IsEmpty() || !args[2]->IsFunction()) {
        return NanThrowError("ReadLine needs a complete callback argument");
    }

    if (sReadLine) {
        return NanThrowError("Can only have one ReadLine object at a time");
    }

    String::Utf8Value prompt(args[0]);
    ReadLine* obj = new ReadLine(*prompt);
    NanAssignPersistent(obj->lineCallback, Handle<v8::Function>::Cast(args[1]));
    NanAssignPersistent(obj->completeCallback, Handle<v8::Function>::Cast(args[2]));
...
void ReadLine::RegisterModule(Handle<Object> target)
{
    if (historyFile.empty()) {
        const char *home = getenv("HOME");
        if (home) {
            historyFile = home;
            historyFile += "/.jsh/history";
            read_history(historyFile.c_str());
        }
    }
    NanScope();

    auto tpl = NanNew<v8::FunctionTemplate>(ReadLine::New);
    NanAssignPersistent(constructor, tpl);

    auto name = NanSymbol("ReadLine");
...
NODE_MODULE(ReadLine, ReadLine::RegisterModule);

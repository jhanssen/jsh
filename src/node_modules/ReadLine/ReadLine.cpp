...
public:
    ...
    SendRequest(char* t, int s, int e)
        : type(Complete), data(t), start(s), end(e), completion(0)
    {
    }
...
void ReadLine::RunCallback(uv_async_t* handle, int /*status*/)
{
    ...
    case SendRequest::Complete: {
        char** c = sReadLine->handleComplete(req->data, req->start, req->end);
        UVMutexLocker locker(*mutex);
        completing = false;
        req->completion = c;
        compCond->signal();
...
{
    UVMutexLocker locker(*mutex);
    SendRequest* req = new SendRequest(const_cast<char*>(text), start, end);
    completing = true;
...
char** ReadLine::handleComplete(char* text, int start, int end)
{
    HandleScope scope;

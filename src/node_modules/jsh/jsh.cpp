...
Handle<Value> JSH::execSync(const Arguments& args)
{
    ...
    for (;;) {
        ...
        eintrwrap(s, ::select(stdoutPipe[0] + 1, &rd, 0, 0, 0));
        if (s <= 0) {
            // bust!
        ...
        if (FD_ISSET(stdoutPipe[0], &rd)) {
            ...
            if (s < 0) {
                // bust!
                return ThrowException(Exception::TypeError(String::New("JSH.execSync read failed")));
            }
            if (s == 0) {
                // done?
                break;
    ...
    return (data.empty() ? Undefined() : String::New(data.c_str(), data.size()));
}

...
void JSH::init(Handle<Object> target)
{
    ...
    NODE_SET_PROTOTYPE_METHOD(constructor, "setupShell", setupShell);
    NODE_SET_PROTOTYPE_METHOD(constructor, "isExecutable", isExecutable);
    NODE_SET_PROTOTYPE_METHOD(constructor, "execSync", execSync);
    NODE_SET_PROTOTYPE_METHOD(constructor, "flockSync", flockSync);
    NODE_SET_PROTOTYPE_METHOD(constructor, "stdout", writeStdout);
    NODE_SET_PROTOTYPE_METHOD(constructor, "stderr", writeStderr);

    target->Set(name, constructor->GetFunction());
}

static inline void writeFile(FILE* f, const Arguments& args)
{
...
Handle<Value> JSH::writeStdout(const Arguments& args)
{
    HandleScope scope;
    writeFile(stdout, args);
    return Undefined();
}

Handle<Value> JSH::writeStderr(const Arguments& args)
{
    HandleScope scope;
    writeFile(stderr, args);
    return Undefined();
}
...
Handle<Value> JSH::execSync(const Arguments& args)
{
    ...
    if (args[1].IsEmpty() || !args[1]->IsArray()) {
        return ThrowException(Exception::TypeError(String::New("JSH.execSync takes an array argument")));
    }
...
Handle<Value> JSH::flockSync(const Arguments& args)
{
    HandleScope scope;

    if (args.Length() != 2) {
        return ThrowException(Exception::TypeError(String::New("JSH.flockSync takes a file descriptor and a mode argument")));
    }
    if (args[0].IsEmpty() || !args[0]->IsInt32()) {
        return ThrowException(Exception::TypeError(String::New("JSH.flockSync takes a file descriptor argument")));
    }
    if (args[1].IsEmpty() || !args[1]->IsArray()) {
        return ThrowException(Exception::TypeError(String::New("JSH.flockSync takes a an array of strings argument")));
    }
    ...
    for (size_t i=0; i<modes->Length(); ++i) {
        ...
        if (!strcasecmp(*mode, "shared")) {
            ...
            return ThrowException(Exception::TypeError(String::New("JSH.flockSync unrecognized mode, available modes are: 'shared', 'exclusive', 'unlock' and 'nonblocking'")));
    ...
    if (::flock(args[0]->ToInt32()->Value(), operation) != 0) {
        ...
        return ThrowException(Exception::TypeError(String::New(buf, w)));
    }
    return True();
}
...
Handle<Value> JSH::setupShell(const Arguments& args)
{
    HandleScope scope;
    JSH* obj = ObjectWrap::Unwrap<JSH>(args.This());

...
Handle<Value> JSH::flockSync(const Arguments& args)
{
    ...
    if (args[1].IsEmpty() || !args[1]->IsUint32()) {
        return ThrowException(Exception::TypeError(String::New("JSH.flockSync takes a uint mode argument")));
    }

    if (::flock(args[0]->ToInt32()->Value(), args[1]->ToUint32()->Value()) != 0) {
        char buf[1024];
